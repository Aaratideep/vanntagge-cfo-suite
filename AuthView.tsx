...
const EMPLOYEE_EMAILS = ['aarati123@gmail.com'];
...
export const AuthView: React.FC = () => {
  ...
  const handleGoogleAuth = async () => {
    ...
    if (isFirebaseConfigured && auth && db) {
      ...
      // Sandbox fallback
      alert("Google Auth requires a valid Firebase configuration. Switching to sandbox demo.");
      loginUser('sarah.jenkins@vanntagge.com');
      setLoading(false);
    }
  };
          ...
          <div className="w-full max-w-[400px] mx-auto">
            ...
            {authMode === 'login' && (
              <div>
                ...
                {/* Quick Role Selector */}
                <div className="mb-6 grid grid-cols-2 gap-2 p-1 bg-slate-100 rounded-xl text-xs font-semibold">
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className={`py-2 px-3 rounded-lg text-center transition-all flex items-center justify-center gap-1.5 ${email.includes('admin') || email.includes('006')
                  ...
                  <button
                    ...
                    className={`py-2 px-3 rounded-lg text-center transition-all flex items-center justify-center gap-1.5 ${email.includes('aarati123') || email.includes('priya') || email.includes('employee')
